...
import { QueryResult, Record } from './typeDefs.js';

type SingleRecordQueryInputs = {
  conn: Connection;
  query: string;
...
const singleRecordQuery = async ({
  ...
  const result = (await retry(async () => (tooling ? conn.tooling.query(query) : conn.query(query)), {
    maxAttempts: 5,
    delay: 1000,
    factor: 2,
  })) as QueryResult;
